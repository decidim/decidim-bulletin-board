...
Among the typical Rails ENV vars (such as `SECRET_KEY_BASE`), it is important to define the private key for this instance of the bulletin board:

Keep reading to see how to generate the private key.

Also,it is important to have an additional docker instance running, with the same ENVs, a sidekiq instance. This is very important as they are many background jobs. If using the provided [entrypoint](docker-entrypoint.sh), you can use simply the ENV `RUN_SIDEKIQ` in the worker:
...
The parameter that defines the identity of a bulletin board, is its private key. This is simply an RSA type of key generate with the OpenSSL library. For convenience you can generate it with the following command:

```bash
bin/rails client:generate_identification_private_key
```

You must run this command inside the `bulletin_board/server` folder or, if using docker-compose (you need it up an running first), access to the bash console with `docker-compose exec app bash` and run it there.
...
This command will produce and output like this:
