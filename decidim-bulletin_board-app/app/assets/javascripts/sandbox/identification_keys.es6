...
((exports) => {
  class IdentificationKeys {
    ...
    async upload(event, alwaysMatches = false) {
      ...
      return new Promise((resolve, reject) => {
        if (event) {
          this.onUploadInputChange(event, resolve, reject, alwaysMatches);
        } else {
    ...
    onUploadInputChange(event, resolve, reject, alwaysMatches) {
      ...
      reader.onload = (readEvent) => {
        ...
        return this.crypto.subtle
          ...
          .then((privateKey) => {
            ...
            if (
              alwaysMatches ||
              this._matchesStoredPublicKey(uploadedPublicKey)
            ) {
    ...
    async sign(payload) {
      ...
      const data = `${this.jwtHeader}.${this._encode64(
