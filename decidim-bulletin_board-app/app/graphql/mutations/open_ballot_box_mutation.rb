...
module Mutations
  class OpenBallotBoxMutation < BaseMutation
    ...
    def resolve(message_id:, signed_data:)
      ...
      result = { error: "There was an error opening the election's ballot box." }

      OpenBallotBox.call(authority, message_id, signed_data) do
        on(:ok) do |election|
