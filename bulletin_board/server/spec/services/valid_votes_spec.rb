...
RSpec.describe ValidVotes do
  ...
  let(:some_votes) { create_list(:log_entry, 10, :vote, election:) }
  ...
  context "when there are duplicated votes" do
    let(:replacement) { create(:log_entry, :vote, election:, voter_id: some_votes.first.author_unique_id) }
  ...
  context "when there are in_person votes" do
    let(:replacement) { create(:log_entry, :in_person_vote, election:, voter_id: some_votes.first.author_unique_id) }
    let(:create_votes) { some_votes && replacement }
