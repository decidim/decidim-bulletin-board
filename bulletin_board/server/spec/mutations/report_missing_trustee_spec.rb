...
module Mutations
  RSpec.describe ReportMissingTrusteeMutation, type: :request do
    ...
    let(:query) do
      <<~GQL
        mutation($messageId: String!, $signedData: String!) {
          reportMissingTrustee(messageId: $messageId, signedData: $signedData) {
            pendingMessage {
              id
              client {
                id
    ...
    let(:payload) { build(:report_missing_trustee_message, election: election) }
    ...
    it "enqueues a tally job to process the message", :jobs do
      subject
      expect(ReportMissingTrusteeJob).to have_been_enqueued
    end

    it "returns a pending message" do
      subject
      json = JSON.parse(response.body, symbolize_names: true)
      data = json.dig(:data, :reportMissingTrustee)
